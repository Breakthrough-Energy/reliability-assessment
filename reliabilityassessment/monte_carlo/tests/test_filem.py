...
from reliabilityassessment.monte_carlo.filem import _filem, filem


def test_filem():
    # Note that: based on the original logic and design of EVNT list (array),
    # its length is multiple of 4 and its last 4 slots are always zeros;

    for f in (filem, _filem):
        ...
        NUMINQ, MFA, IPOINT = f(MFA, ATRIB, NUMINQ, IPOINT, EVNTS)
        assert NUMINQ == 1
        assert MFA == 4
        assert IPOINT == 0
